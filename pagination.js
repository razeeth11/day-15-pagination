let nxt_but = document.querySelector('.btn-nxt')
nxt_but.addEventListener('click', (event) =>{
  ...
  for(var i=1; i<=7;i++){
    ...
    var but_Num = document.querySelector(`.btn-${i}`)
    var attribute_get = parseInt(but_Num.getAttribute('for')) 


   if(attribute_get <= 84){

    // Condition for show upto 91 pages 

    var new_value = attribute_get + 7;
    but_Num.setAttribute('for', new_value)
    but_Num.innerText = new_value;
   }
  }
...
let prev_but = document.querySelector('.btn-prev')
prev_but.addEventListener('click', (event) =>{
  event.preventDefault();
